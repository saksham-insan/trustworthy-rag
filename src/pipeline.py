...
def run_pipeline(question: str, lang: str, index_condition: str = "mono") -> dict:
...
def log_to_db(record: dict):
    ...
    conn.execute(
        """
        INSERT INTO logs (
            timestamp, question, language, index_condition,
...
def main():
    if len(sys.argv) < 3:
        print('Usage: python src/pipeline.py "your question" <lang: en|hi|bn> [mono|multi]')
        sys.exit(1)
    ...
    print("Running full pipeline (retrieve -> generate -> verify)...\n")

    record = run_pipeline(question, lang, index_condition)
